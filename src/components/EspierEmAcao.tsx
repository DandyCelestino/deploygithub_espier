...
const items: Item[] = [
  {
    ...
    caption: "Equipe uniformizada, frota identificada e múltiplas frentes simultâneas — CFTV, controle de acesso e cabeamento, executados no mesmo dia.",
...
const EspierEmAcao = () => (
  <section id="em-acao" className="py-20 lg:py-32 bg-background relative overflow-hidden">
    {/* Glow decorativo */}
    ...
    <div className="section-container relative">
      <div className="text-center max-w-3xl mx-auto mb-14 sm:mb-16">
        ...
        <p className="mt-4 text-lg sm:text-xl font-semibold text-foreground/90">
          Conectando você ao que importa — com técnica, presença e excelência.
        </p>
        <p className="mt-3 text-muted-foreground text-base sm:text-lg leading-relaxed">
          Veja de perto a rotina dos nossos times: projetos executados com padrão corporativo,
          equipe identificada, frota própria e tecnologia de ponta atendendo condomínios,
          empresas e residências de alto padrão.
        </p>

        {/* Trust strip */}
        <div className="mt-7 flex flex-wrap justify-center items-center gap-x-6 gap-y-3 text-xs sm:text-sm text-muted-foreground">
          <span className="inline-flex items-center gap-1.5"><ShieldCheck className="w-4 h-4 text-primary" /> Equipe uniformizada</span>
          <span className="w-1 h-1 rounded-full bg-border" />
          <span className="inline-flex items-center gap-1.5"><Wrench className="w-4 h-4 text-primary" /> Frota própria identificada</span>
          <span className="w-1 h-1 rounded-full bg-border" />
          <span className="inline-flex items-center gap-1.5"><Camera className="w-4 h-4 text-primary" /> Obras documentadas</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 auto-rows-[200px] sm:auto-rows-[220px] lg:auto-rows-[240px] gap-4">
        {items.map((it, i) => (
          <figure
            key={i}
            className={`group relative overflow-hidden rounded-2xl border border-border bg-card shadow-sm hover:shadow-[0_25px_60px_-20px_hsl(0_80%_50%/0.35)] hover:border-primary/40 transition-all duration-500 ${it.span ?? ""}`}
          >
            <img
              src={it.img}
              alt={it.title}
