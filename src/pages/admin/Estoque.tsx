    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500">Controle de materiais, entradas e saídas.</p>
        </div>
        {canManage && (
          <Dialog open={openItem} onOpenChange={setOpenItem}>
            ...
            <DialogContent className="bg-white border-gray-200">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div><Label className="text-gray-600">Qtd. Inicial</Label><Input type="number" className="bg-white border-gray-300 text-gray-900" value={formItem.quantidade} onChange={(e) => setFormItem({ ...formItem, quantidade: e.target.value })} /></div>
      ...
      <Tabs defaultValue="itens" className="w-full">
        <TabsList className="bg-white border border-gray-200">
          <TabsTrigger value="itens" className="text-gray-600 data-[state=active]:text-gray-900 data-[state=active]:bg-gray-100">Itens em Estoque</TabsTrigger>
          <TabsTrigger value="movimentacoes" className="text-gray-600 data-[state=active]:text-gray-900 data-[state=active]:bg-gray-100">Movimentações</TabsTrigger>
        </TabsList>

        <TabsContent value="itens" className="mt-4">
          <div className="flex items-center gap-2 mb-4">
            <Search className="h-4 w-4 text-gray-400" />
            <Input placeholder="Buscar item..." className="max-w-sm bg-white border-gray-300 text-gray-900 placeholder:text-gray-400" value={search} onChange={(e) => setSearch(e.target.value)} />
          </div>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="border-gray-200">
                    ...
                    <TableHead className="text-gray-600">Qtd.</TableHead>
                    <TableHead className="text-gray-600">Unidade</TableHead>
                    <TableHead className="text-gray-600">Mín.</TableHead>
                ...
                <TableBody>
                  {isLoading ? (
                    ...
                    <TableRow><TableCell colSpan={8} className="text-center text-gray-500">Nenhum item encontrado.</TableCell></TableRow>
                  ) : filtered.map((item) => (
                    <TableRow key={item.id} className="border-gray-200">
                      <TableCell className="text-gray-600 font-mono text-xs">{item.codigo || "—"}</TableCell>
                      <TableCell className="text-gray-900 font-medium">{item.descricao}</TableCell>
                      <TableCell>
                        <span className={item.quantidade <= item.quantidade_minima ? "text-red-600 font-bold" : "text-gray-900"}>
                          {item.quantidade}
                        </span>
                      </TableCell>
                      <TableCell className="text-gray-600">{item.unidade}</TableCell>
                      <TableCell className="text-gray-500">{item.quantidade_minima}</TableCell>
                      <TableCell className="text-gray-600">{item.localizacao || "—"}</TableCell>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="border-gray-200">
                    ...
                    <TableHead className="text-gray-600">Obs.</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {movimentacoes.length === 0 ? (
                    <TableRow><TableCell colSpan={6} className="text-center text-gray-500">Nenhuma movimentação registrada.</TableCell></TableRow>
                  ) : movimentacoes.map((mov) => (
                    <TableRow key={mov.id} className="border-gray-200">
                      <TableCell className="text-gray-500 text-xs">{new Date(mov.created_at).toLocaleString("pt-BR")}</TableCell>
                      <TableCell>
                        <Badge className={mov.tipo === "entrada" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}>
                          {mov.tipo === "entrada" ? "Entrada" : "Saída"}
                        </Badge>
                      </TableCell>
                      <TableCell className="text-gray-900">{getItemName(mov.item_id)}</TableCell>
                      <TableCell className="text-gray-900">{mov.quantidade}</TableCell>
                      <TableCell className="text-gray-600">{mov.tecnico_nome || "—"}</TableCell>
                      <TableCell className="text-gray-500 text-xs">{mov.observacao || "—"}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
      ...
      <Dialog open={openMov} onOpenChange={setOpenMov}>
        <DialogContent className="bg-white border-gray-200">
          ...
          <div className="space-y-4">
            <p className="text-gray-600 text-sm">Item: <strong className="text-gray-900">{selectedItem?.descricao}</strong></p>
            <p className="text-gray-500 text-sm">Estoque atual: {selectedItem?.quantidade} {selectedItem?.unidade}</p>
            <div><Label className="text-gray-600">Quantidade *</Label><Input type="number" className="bg-white border-gray-300 text-gray-900" value={formMov.quantidade} onChange={(e) => setFormMov({ ...formMov, quantidade: e.target.value })} /></div>
